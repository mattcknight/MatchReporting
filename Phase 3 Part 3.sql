...
JOIN [ProfileReadData].[dbo].[Users] u WITH (NOLOCK) ON u.UserID = sal.UserID 
	...
	AND u.SignupDt >= '2014-05-19 12:30:00' -- remove fraud
	AND u.SignupDt < '2014-05-30 12:00:00'
...
SELECT DISTINCT
	...
	,UserGroup = CASE
		WHEN pIII.ScoreGroup = 'Group 01' THEN
			CASE   
				WHEN pIII.Decile IN ('0','1','2','3','4','5','6','7') THEN '01 - Control (50% lvl)'
				WHEN pIII.Decile IN ('8','9','A','B','C','D','E','F') THEN '02 - 7.5% Up (50% lvl)'
				END
		WHEN pIII.ScoreGroup = 'Group 02' THEN
			CASE   
				WHEN pIII.Decile IN ('0','1','2','3','4','5','6','7') THEN '03 - Control (50% lvl)'
				WHEN pIII.Decile IN ('8','9','A','B','C','D','E','F') THEN '04 - 5.0% Up (50% lvl)'
				END
		WHEN pIII.ScoreGroup = 'Group 03' THEN '05 - Control'
		WHEN pIII.ScoreGroup = 'Group 04' THEN '06 - Control'
		WHEN pIII.ScoreGroup = 'Group 05' THEN '07 - Control'
		END
	,pIII.SignupDt
    ,sal.LogDateTime
INTO #allRCViewers
...
AND sal.LogDateTime >= '2014-05-19 12:30:00'
AND sal.LogDateTime < '2014-05-30 12:00:00'
...
WHERE t.CreateDt >= '2014-05-19 12:30:00'
AND t.CreateDt < '2014-05-30 12:00:00'
...
INNER JOIN BillingData.dbo.Trx c ON a.userid = c.userid 
	...
	AND c.CreateDt >= '2014-05-19 12:30:00'
	AND c.CreateDt < '2014-05-30 12:00:00'
GROUP BY ScoreGroup, UserGroup
ORDER BY ScoreGroup, UserGroup

DELETE FROM WorkDB.dbo.MK_PriceTest3_Phase3;
INSERT INTO WorkDB.dbo.MK_PriceTest3_Phase3 (ScoreGroup,Cohort,RCViewers,PmntViewers,Subs,RC_Pmnt,Pmnt_Sub,RC_Sub,TotalCash,ARPU,Resignations,ResignRate)
SELECT 
...
SELECT 
	...
	,[Resignations]
	,[Resign %] = ResignRate
FROM WorkDB.dbo.MK_PriceTest3_Phase3 
...
--TRUNCATE TABLE WorkDB.dbo.MK_PriceTest3_Phase3;
--DROP TABLE WorkDB.dbo.MK_PriceTest3_Phase3;
--CREATE TABLE WorkDB.dbo.MK_PriceTest3_Phase3 (
